...
fn dp_base_case(dp: &mut  Vec<Vec<u64>>, record: &String, numbers: &Vec<usize>) {
    ...
    for start in (0..n).rev() {
        ...
        if record[start..=start].contains(".") {
        ...
        } else if record[start..=start].contains("?") {
            if n-start >= group_size && (start == 0 || !record[start-1..=start-1].contains("#")) {
                if !record[start..end].contains(".") {
                    if !record[start..end].contains("#"){
                        if dp[m-1][start+1] > 0 {
                            if !record[end..n].contains("#") {
                                dp[m-1][start] = 1 + dp[m-1][start+1];
                            } else {
                                dp[m-1][start] = dp[m-1][start+1];
                            }
                        } else if end == n || !record[end..n].contains("#") {
                            dp[m-1][start] = 1
                        }
                    } else {
                        if !record[end..n].contains("#") {
            ...
            if n-start >= group_size && (start == 0 || !record[start-1..=start-1].contains("#")){
                if !record[start..end].contains(".") && !record[end..n].contains("#"){
                    dp[m-1][start] = 1;
                }
            }
        }
...
pub fn twostar_solution() {
    ...
    for line in read_to_string("src/day12twostar/input").unwrap().lines() {
        ...
        let value = process_record(expanded_str, expanded_numbers);
        writeln!(file, "{}", value).expect("Error writing to file.");
        total += value;
    }
